...
def validate_input(frame: pd.DataFrame) -> None:
    ...
    if len(frame) < MIN_SAMPLES:
        raise ValueError(
            f"Input has {len(frame)} samples; at least {MIN_SAMPLES} are required."
        )
...
def clean_data(frame: pd.DataFrame) -> np.ndarray:
    """Convert to finite float data using deterministic within-file imputation."""
    numeric = frame.apply(pd.to_numeric, errors="coerce").replace(
        [np.inf, -np.inf], np.nan
    )
    if numeric.isna().all(axis=0).any():
        bad = numeric.columns[numeric.isna().all(axis=0)].tolist()
        raise ValueError(f"Entirely invalid numeric channel(s): {bad[:3]}")
    numeric = numeric.interpolate(axis=0, limit_direction="both")
    numeric = numeric.fillna(numeric.median(axis=0))
    values = numeric.to_numpy(dtype=np.float32, copy=True)
    if not np.isfinite(values).all():
        raise ValueError("Input still contains non-finite values after cleaning.")
    return values
...
def extract_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for (side, kind), indices in groups.items():
        ...
        for metric_name, metric in (
            ("std", std), ("rms", rms), ("abs_peak", abs_peak),
            ("ptp", ptp), ("kurtosis", kurtosis), ("crest", crest),
        ):
    ...
    for kind in ("vibration", "shock"):
        ...
        for key_i in [key for key in result if key.startswith(prefix_i)]:
            ...
            if key_ii in result:
                result[f"side_contrast_{kind}_{suffix}"] = result[key_i] - result[key_ii]
